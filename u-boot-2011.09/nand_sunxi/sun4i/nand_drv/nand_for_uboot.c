...
int NAND_PhyInit(void)
{
	...
	{
		readop.page = i;
	    ret = NAND_PhyRead (&readop);
	    if( ret != 0 )
		{
		   OSAL_printf("reading block %u page %u failed.\n", readop.block,readop.page );
		}
		else if(*((__u32 *)main_buf ) != 0x03020100)
		{
			OSAL_printf("page %d flag %x error.\n", readop.page, *((__u32 *)main_buf ) );
		}
		else
		{
			flag_get_ratio=1;
	...
	if(flag_get_ratio)
	{
		nand_good_blk_ratio = *((__u32 *)main_buf + 1);

		NAND_SetValidBlkRatio(nand_good_blk_ratio);
		OSAL_printf("nand_good_blk_ratio %u \n", nand_good_blk_ratio);
	}
	else
		OSAL_printf("get block ratio from block7 fail ,keep default!\n");
	NAND_Free(main_buf,1024*16);
